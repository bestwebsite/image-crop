...
Plugin URI: https://github.com/bestwebsite/image-crop
Description: Plugin allows you to manually crop all the image sizes..
Version: 1.0
Author: bestwebsite
Author URI: https://github.com/bestwebsite
*/

define('mic_VERSION', '1.12');

include_once(dirname(__FILE__) . '/lib/bestwebsiteImageCropSettingsPage.php');
...
function mic_init_plugin() {
	...
	include_once(dirname(__FILE__) . '/lib/bestwebsiteImageCrop.php');

	load_plugin_textdomain('microp', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );

	$bestwebsiteImageCrop = bestwebsiteImageCrop::getInstance();
	add_action( 'admin_enqueue_scripts', array($bestwebsiteImageCrop, 'enqueueAssets') );
	$bestwebsiteImageCrop->addEditorLinks();
...
function mic_ajax_editor_window() {
	include_once(dirname(__FILE__) . '/lib/bestwebsiteImageCropEditorWindow.php');
	$bestwebsiteImageCropEditorWindow = bestwebsiteImageCropEditorWindow::getInstance();
	$bestwebsiteImageCropEditorWindow->renderWindow();
	exit;
}
...
function mic_ajax_crop_image() {
	$bestwebsiteImageCrop = bestwebsiteImageCrop::getInstance();
	$bestwebsiteImageCrop->cropImage();
	exit;
}
